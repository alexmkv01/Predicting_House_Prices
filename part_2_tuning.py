...
def plot_learning_curve(training_errors, val_errors):
    ...
    plt.xlabel("Epochs")
    plt.ylabel("RMSE")threshold=0.0001
...
def RegressorHyperParameterSearch(x_train, y_train, config): 
    ...
    classifier = GridSearchCV(estimator = regressor, cv=3, param_grid = config, verbose=3, refit="neg_root_mean_squared_error", scoring=["neg_root_mean_squared_error", "r2"], return_train_score=True)
    classifier.fit(x_train, y_train)
    
    print(classifier.best_params)
    print(classifier.best_score)
    print(classifier.best_estimator)
